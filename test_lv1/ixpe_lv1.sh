...
DATA_FOLDER=$PREFIX'data/leakage/'$OBS'/'
DET='3'
INFILE='event_l1/ixpe'$OBS'_det'$DET'_evt1_v02'
...
ixpegaincorrtemp infile=$DATA_FOLDER"$INFILE"_recon.fits outfile=$DATA_FOLDER"$INFILE"_recon_gain.fits hkfile="$DATA_FOLDER"hk/ixpe"$OBS"_all_pay_132"$DET"_v01.fits clobber=True logfile=recon.log
ixpechrgcorr infile=$DATA_FOLDER"$INFILE"_recon_gain.fits outfile=$DATA_FOLDER"$INFILE"_recon_gain_corr.fits initmapfile="$CALDB"/data/ixpe/gpd/bcf/chrgmap/ixpe_d"$DET"_20170101_chrgmap_01.fits outmapfile=$DATA_FOLDER"$INFILE"_chrgmap.fits phamax=60000.0 clobber=True
...
ftpaste $DATA_FOLDER"$INFILE"'_recon_gain_corr_map.fits[EVENTS][col -DETPHI2;]' $PREFIX'data_leakage_'$OBS'_recon_gx301_det'$DET'___nn_gx301__ensemble.fits[1][col NN_PHI, DETPHI2==NN_PHI; NN_WEIGHT, W_NN==NN_WEIGHT; P_TAIL; FLAG]' $DATA_FOLDER"$INFILE"_recon_nn.fits history=YES clobber=True
...
# Jack changed this from Lawrence's code
ixpeadjmod infile=$DATA_FOLDER"$INFILE"_recon_nn_stokes.fits outfile=$DATA_FOLDER"$INFILE"_recon_nn_stokes_adj.fits clobber=True spmodfile="$PREFIX"caldb/spmod/ixpe_d"$DET"_20170101_spmod_nn.fits
...
ixpedet2j2000 infile=$DATA_FOLDER"$INFILE"_recon_nn_stokes_adj_w.fits outfile=$DATA_FOLDER"$INFILE"_recon_nn_stokes_adj_w_j2000.fits attitude="$DATA_FOLDER"hk/ixpe"$OBS"_det"$DET"_att_v0"$ATTNUM".fits clobber=True
ixpeaspcorr infile=$DATA_FOLDER"$INFILE"_recon_nn_stokes_adj_w_j2000.fits clobber=True n=300 att_path="$DATA_FOLDER"hk/ixpe"$OBS"_det"$DET"_att_v0"$ATTNUM".fits
